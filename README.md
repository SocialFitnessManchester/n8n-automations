...
| [`glofox-lead-poll/`](./glofox-lead-poll/) | The **ongoing, automatic** counterpart to `glofox-lead-sync`: every **6 hours** it loops through every GHL-ready studio and pushes **new** Glofox leads into Go High Level (type `lead`). Built as a conductor + per-studio worker; per-studio creds come from `studio_config` (nothing hardcoded). Inactive until switched on. |
...
| Lead Poll — conductor (all studios, 6h) | [Open](https://automation.social-fitness.com/workflow/P5xUqBY9rRaEJ7XK) _(inactive — switch on to go live)_ |
| Lead Poll — single-studio worker (sub) | [Open](https://automation.social-fitness.com/workflow/btcLAW4tzEymNG9s) _(called by the conductor)_ |
